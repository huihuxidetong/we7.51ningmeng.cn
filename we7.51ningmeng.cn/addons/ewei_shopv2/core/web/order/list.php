			...
			else 
			{
				...
				else 
				{
					...
					else 
					{
						...
						{
							$condition .= " AND ( locate(:keyword,a.realname)>0 or locate(:keyword,a.mobile)>0 or locate(:keyword,a.shopname)>0 or locate(:keyword,o.carrier)>0 or locate(:keyword,o.address)>0)";
							$priceCondition .= " AND ( locate('" . $_GPC["keyword"] . "',a.realname)>0 or locate('" . $_GPC["keyword"] . "',a.mobile)>0 or locate('" . $_GPC["keyword"] . "',a.shopname)>0 or locate('" . $_GPC["keyword"] . "',o.carrier)>0 or locate('" . $_GPC["keyword"] . "',o.address)>0 ";
						}
						else 
						{
										...
										else 
										{
											...
											else 
											{
												...
												else 
												{
													...
													{
														if( $searchfield == "goodsoptiontitle" ) 
														{
		...
		else 
		{
			...
			if( !empty($list) ) 
			{
				...
				foreach( $list as $key => &$value ) 
				{
					...
					//add@20190529 for new filed[shopname]
					$list[$key]["ashopname"] = $addressid_array[$value["addressid"]]["ashopname"];
					//end add
		...
		if( !empty($list) ) 
		{
			...
			foreach( $list as &$value ) 
			{
				...
				else 
				{
					...
					//add@20190529 for new filed[shopname]
					$value["shopname"] = ($isarray ? $address["shopname"] : $value["ashopname"]);
					//end add
			...
			else 
			{
				...
				else 
				{
					...
					else 
					{
						...
						{
							$redis = redis();
							$redis_key_total = $_W["uniacid"] . "_ewei_shop_order_list_total";
							$redis_key_totalmoney = $_W["uniacid"] . "_ewei_shop_order_list_totalmoney";
							if( !is_error($redis) ) 
							{
								if( $redis->get($redis_key_total) != false && $redis->get($redis_key_totalmoney) != false ) 
								{
								...
								else 
								{
									...
									$redis->set($redis_key_total, $t["count"], array( "nx", "ex" => "300" ));
									$redis->set($redis_key_totalmoney, $t["sumprice"], array( "nx", "ex" => "300" ));
									$total = $t["count"];
